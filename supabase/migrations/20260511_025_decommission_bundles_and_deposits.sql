-- 1. Remove bundle_id from bookings
ALTER TABLE bookings DROP COLUMN IF EXISTS bundle_id;

-- 2. Drop bundle functions
DROP FUNCTION IF EXISTS fn_bundle_total_price(UUID);

-- 3. Update fn_create_booking to remove bundle parameter and logic
CREATE OR REPLACE FUNCTION fn_create_booking(
  p_game_id           UUID,
  p_bundle_id         UUID, -- Keeping parameter for ABI compatibility if needed, but ignoring it
  p_date              DATE,
  p_start_time        TIME,
...
BEGIN
  ...
  -- 3. Determine base pricing (Bundles DECOMMISSIONED)
  ...
  -- 5. Deposit calculation (from venue_settings) - Force disabled as per requirement
  -- But we keep logic for backward compatibility if settings change later
  ...
  INSERT INTO bookings (
    booking_code, game_id, special_mission_id,
    ...
    v_booking_code, p_game_id, p_special_mission_id,
...
-- 4. Drop bundles table
DROP TABLE IF EXISTS bundles;

-- 5. Remove 'manage_bundles' permission
DELETE FROM permissions WHERE key = 'manage_bundles';
