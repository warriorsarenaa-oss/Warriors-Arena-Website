      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="flex flex-col gap-1.5">
          <label
            ...
            className="text-wa-text/60 font-mono text-[10px] uppercase tracking-widest flex items-center gap-2"
          >
          ...
          />
          {errors.customer_name && (
            <span id="name-error" role="alert" className="text-wa-error text-xs font-mono">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          />
          {errors.customer_phone && (
            <span id="phone-error" role="alert" className="text-wa-error text-xs font-mono">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {errors.customer_email && (
            <span id="email-error" role="alert" className="text-wa-error text-xs font-mono">
              {errors.customer_email.message}
            </span>
          )}
        </div>

        {/* 4. Notes — 2 rows to stay compact on mobile */}
        <div className="flex flex-col gap-1.5">
          <label
      ...
      <div className="flex flex-col gap-1.5 pt-3 border-t border-wa-gray/20">
        <div className="relative">
          ...
          >
            {/* Custom checkbox */}
            <div
              className={`w-5 h-5 border-2 shrink-0 flex items-center justify-center transition-all duration-200 ${
                isChecked
            ...
            >
              {isChecked && <CheckCircle2 className="w-3.5 h-3.5 text-wa-bg" />}
            </div>
      ...
      {showPolicy && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-wa-black/95 backdrop-blur-md">
          <WAPanel hot className="max-w-md w-full p-8 flex flex-col items-center text-center gap-6 relative">
            ...
            <WAButton variant="orange" className="w-full mt-2" onClick={() => setShowPolicy(false)}>
              UNDERSTOOD
            </WAButton>
          </WAPanel>
        </div>
