...
export const POST = requirePermission(async (
  ...
  try {
    ...
    const updateData: any = {
      ...
      deposit_status: refund_deposit ? 'refunded' : 'forfeited',
      updated_at: new Date().toISOString(),
    };

    if (refund_deposit) {
      updateData.deposit_refunded_at = new Date().toISOString();
      updateData.deposit_refunded_by = user.id;
      updateData.deposit_refund_reason = reason;
    }
